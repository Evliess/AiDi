import { app } from '../../app';
import { openId, token, updateLeftCountByPhone, updateScoreByPhone,
...
Page({
  ...
  async play() {
    try{
      ...
      await updateLeftCountByPhone(url, openId, token);
  ...
  async onLoad() {
    try {
      const url = "/playitems";
      const findPlayItemsRes = await findPlayItems(url, openId, token);
      const findItems:any = [];
